...
<head>
    ...
    <title>{{ config('app.name') }} - {{ __('messages.register.title') }}</title>
            ...
            <div class="bg-white rounded-2xl p-6 sm:p-8 lg:p-10 shadow-xl w-full max-w-4xl" data-aos="fade-up">
                <div class="flex flex-col lg:flex-row gap-8">
                    <div class="lg:w-2/5 flex flex-col items-center justify-center gap-4 text-center">
                        ...
                        <p class="text-gray-700 text-lg">{{ __('messages.register.create_account') }}</p>
                    </div>
                    <div class="lg:w-3/5">
                        @if($errors->any())
                        ...
                        @endif
                        <h1 class="text-3xl sm:text-4xl font-bold text-gray-900 mb-6">{{ __('messages.register.title') }}</h1>
                        <form novalidate action="{{ route('register.post') }}" method="post" class="flex flex-col gap-4" aria-labelledby="register-title">
                            @csrf
                            <div class="flex flex-col gap-2">
                                <label for="name" class="text-gray-700 font-medium">{{ __('messages.register.full_name') }}</label>
                                <input class="w-full rounded-xl border-2 p-3 @error('name') border-red-500 bg-red-50 @else border-gray-200 focus:border-gray-900 @enderror transition-colors duration-200" 
                                       type="text" 
                                       name="name" 
                                       id="name"
                                       value="{{ old('name') }}"
                                ...
                                @error('name')
                                    <span id="name-error" class="text-red-700 text-sm">{{ $message }}</span>
                                @enderror
                            </div>

                            <div class="flex flex-col gap-2">
                                <label for="email" class="text-gray-700 font-medium">{{ __('messages.register.email') }}</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label for="password" class="text-gray-700 font-medium">{{ __('messages.register.password') }}</label>
                            ...
                            <div class="flex flex-col gap-2">
                                <label for="password_confirmation" class="text-gray-700 font-medium">{{ __('messages.register.confirm_password') }}</label>
                            ...
                            <button type="submit" 
                                    ...
                                    aria-label="{{ __('messages.register.submit') }}">
                        ...
                        <a href="{{ route('login') }}" 
                           ...
                           aria-label="{{ __('messages.register.already_account') }}">
