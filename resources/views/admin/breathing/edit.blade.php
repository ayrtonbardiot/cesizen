...
@section('title', __('messages.admin.breathing.actions.edit'))

@section('content')
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-6">{{ __('messages.admin.breathing.actions.edit') }}</h2>

            <form action="{{ route('admin.breathing.update', $breathing) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $breathing->title) }}" required
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3" required
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $breathing->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="category_id" id="category_id" required
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id', $breathing->category_id) == $category->id ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    @if($breathing->image)
                        <div class="mt-2 mb-4">
                            <img src="{{ Storage::url($breathing->image) }}" alt="{{ $breathing->title }}" class="h-32 w-32 object-cover rounded-lg">
                        </div>
                    @endif
                            ...
                            <div class="step-item flex gap-4 items-start">
                                <div class="flex-1">
                                    <select name="steps[{{ $index }}][type]" required
                                        ...
                                        <option value="inhale" {{ $step['type'] === 'inhale' ? 'selected' : '' }}>{{ __('messages.admin.breathing.form.step_types.inhale') }}</option>
                                        <option value="exhale" {{ $step['type'] === 'exhale' ? 'selected' : '' }}>{{ __('messages.admin.breathing.form.step_types.exhale') }}</option>
                                        <option value="hold" {{ $step['type'] === 'hold' ? 'selected' : '' }}>{{ __('messages.admin.breathing.form.step_types.hold') }}</option>
                                    </select>
                                </div>
                                <div class="flex-1">
                                    <input type="number" name="steps[{{ $index }}][duration]" min="1" required value="{{ $step['duration'] }}" placeholder="{{ __('messages.admin.breathing.form.duration_placeholder') }}"
                                        class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                </div>
                                <button type="button" class="remove-step text-red-600 hover:text-red-900" {{ count($breathing->steps) === 1 ? 'style=display:none' : '' }}>
                ...
                <div>
                    <label class="inline-flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $breathing->is_active) ? 'checked' : '' }}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let stepCount = {{ count($breathing->steps) }};
